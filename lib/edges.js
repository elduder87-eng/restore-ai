...
import { areCrossCluster } from './domains.js'
 ...
 */
export async function recordCrossTopic(userId, topics, emotionalState, bridge) {
  if (emotionalState === 'confused') return
  ...
  for (let i = 0; i < topics.length; i++) {
    for (let j = i + 1; j < topics.length; j++) {
      ...
      if (typeof bridge === 'string' && bridge.length > 0) {
        patch.last_bridge_text = bridge
      }
      await redis.hset(edgeKey, patch)
    }
  }
}
 ...
 */
export async function recordMastering(userId, edgeId, bridge) {
  if (!edgeId) return
  ...
  if (isTopicPair) {
    ...
    await recordCrossTopic(userId, topics, 'mastering', bridge)
  } else {
  ...
  if (isTopicPair && typeof bridge === 'string' && bridge.length > 0) {
    patch.last_bridge_text = bridge
  }
  await redis.hset(edgeKey, patch)
}
...
export async function getEdge(userId, edgeId) {
  ...
  return {
    ...
    last_bridge_text: raw.last_bridge_text || null
  }
}
 ...
 */
async function fetchAllEdgesForUser(userId) {
  if (!userId) return []
...
/**
 * In-memory TTL cache for getAllEdgesForUser.
 * Keyed by userId. Value is { edges, expiresAt }.
 *
 * The Insights 2.0 UI issues three lens fetches on load
 * (Foundations, Active Threads, Crossroads); this cache lets
 * them share a single Redis scan when they land within TTL.
 */
const EDGES_CACHE_TTL_MS = 30_000
const edgesCache = new Map()

export async function getAllEdgesForUser(userId) {
  if (!userId) return []
  const cached = edgesCache.get(userId)
  const now = Date.now()
  if (cached && cached.expiresAt > now) {
    return cached.edges
  }
  const edges = await fetchAllEdgesForUser(userId)
  edgesCache.set(userId, { edges, expiresAt: now + EDGES_CACHE_TTL_MS })
  return edges
}

/**
 * Invalidates the cached edges for a user. Called by the chat route
 * after any write (recordEngagement / recordCrossTopic / recordMastering /
 * recordPostMasteryEngagement) so subsequent reads reflect the fresh state.
 */
export function invalidateEdgesCache(userId) {
  if (!userId) return
  edgesCache.delete(userId)
}
...
/**
 * Return cross-cluster pair edges for a user, grouped into mastered vs developing.
 * Only includes Topic<->Topic edges where the two topics belong to different clusters.
 * Within-group sort is insertion order for V1 (deferred design decision).
 *
 * @param {string} userId
 * @returns {Promise<{mastered: Edge[], developing: Edge[]}>}
 */
export async function getCrossroads(userId) {
  if (!userId) return { mastered: [], developing: [] }
  ...
  for (const e of edges) {
    if (!e.edgeId.includes('::')) continue

    const [a, b] = e.edgeId.split('::')
    if (!areCrossCluster(a, b)) continue
